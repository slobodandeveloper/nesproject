...
if (!empty($_POST))
{   
    $upload_type = inputParam('upload_type');
    if($upload_type == "profile_image") {
        ...
        if($file != "") {
            ...
            $filelink = "./uploads/Image/".uniqId().uniqId().uniqId().".".$ext;
            ...
            if(strtolower($ext) == "png")
                imagepng($resized, $filelink);
            else
                imagejpeg($resized, $filelink);
        }
        if($filelink != "") {
            $sql = "UPDATE users SET avatar_path='$filelink' WHERE ID='$uid'";
